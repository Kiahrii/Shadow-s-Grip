...
screen_title_image = PhotoImage(file="title_screen_rectangle.png")
...
def display(text):
    text_id = canvas.create_text(95,100, text=text, font=('Times New Roman', 40, 'bold'), fill='white', anchor='nw', justify='center', width=850)  
    return text_id
...
def end_menu():
    ...
    end_menu_button = Button(window, command=title_screen, text='Main Menu', font=('Times New Roman', 20, 'bold'), fg='gold', bg='black', activeforeground='white',activebackground='gray', relief=RAISED, borderwidth=5, padx=5, pady=5)
    end_menu_button.place(x=425, y=510)
    dynamic_widgets.append(end_menu_button)

# Back Button for Credits
def back():
    back_button = Button(window, text='Back', command=title_screen, font=('Wide Latin', 15,), fg='black', bg='gray', relief=RAISED)
...
def timer():
    ...
    if my_timer > 0:
        ...
        canvas.delete('all')
        photo = PhotoImage(file='Image26.png')
...
def riddle_2_choices(): # TIMED
    ...
    riddle_text = Label(window, text='"I speak without a mouth and hear without ears. I have no body, but I come alive with wind. Who am I?"',
                        font=('Lucida Handwriting', 40), fg='gold', bg='black', wraplength=850, relief=RAISED, borderwidth=5, justify='center', padx=10, pady=5)
    riddle_text.place(x=85, y=100)
    dynamic_widgets.append(riddle_text)
    button_A = buttons(window, text='ECHO', command=echo, x=250, y=400)
    button_B = buttons(window, text='SILENCE', command=silence, x=600, y=400)

def silence(): # TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image23.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    reset_timer()
    display("As you proceed, you hear footsteps getting louder behind you.")
    button_A = buttons(window, text='RUN', command=run, x=450, y=410)
    button_B = buttons(window, text='Hide in a dark hollow', command=hide, x=340, y=520)

def leave_magazine(): # TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image19.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')
    ...
    button_A = buttons(window, text='SHOOT', command=shoot_no_magazine, x=250, y=400)
    button_B = buttons(window, text='ESCAPE', command=fail_escape, x=600, y=400)    
...
def two_path(): # TIMED
    ...
    button_A = buttons(window, text='LEFT', command=search, x=250, y=400)
    button_B = buttons(window, text='RIGHT', command=leave_magazine, x=600, y=400)

def left_path2(): #TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image22.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    reset_timer()
    display("As you moved deeper into the maze, a low growl echoes behind you. The atmosphere thickens with tension.")
    button_A = buttons(window, text='Keep moving cautiously', command=two_path, x=300, y=410)
    button_B = buttons(window, text='Hide in a dark hollow', command=hide, x=320, y=520) 
...
def pick_magazine(): # TIMED
    ...
    reset_timer()
    text_id = display(text="You confront the lurking shadow, now armed with a loaded gun! Your heart races as you prepare for worst.")
    canvas.itemconfig(text_id, fill='red')
    canvas.coords(text_id, 90, 250)

    button_A = buttons(window, text='SHOOT', command=shoot_magazine, x=450, y=470)
    button_B = buttons(window, text='Throw the gun at his face', command=throw_gun, x=350, y=550)
...
def run(): # BAD ENDING
    ...
    stop_timer()
    end_menu()
    text_id = display("The footsteps are now loud and distinct closing the distance. Until you've hit a wall and the pursuer pulled you into darkness. GAME OVER")
    canvas.config(text_id, fill='gold')
...
def echo(): # GOOD ENDING
    ...
    stop_timer()
    text_id = display("The light flickered, then vanished, replaced by a vibrant green. As if in response, the wall began to slide. YOU ESCAPED!")
    canvas.itemconfig(text_id, font=('Times New Roman', 18, 'bold'), fill='gold')
    end_menu()
...
def throw_gun(): # BAD ENDING
    ...
    stop_timer()
    text_id = display("He dodged it and lunged towards you, pulling you into darkness.")
    canvas.itemconfig(text_id, fill='red')
    canvas.coords(text_id, 150,250)
    end_menu() 
...
def fail_escape(): # BAD ENDING
    ...
    stop_timer()
    text_id = display("You died trying. GAME OVER")
    canvas.itemconfig(text_id, fill='red')
    canvas.coords(text_id, 150,250)
    end_menu()
...
def shoot_no_magazine(): # BAD ENDING
    ...
    stop_timer()
    text_id = display("You fired the gun but the magazine is empty. The creature lunge towards you, pulling you into darkness. GAME OVER.")
    canvas.itemconfig(text_id, fill='red')
    canvas.coords(text_id, 90, 250)
    end_menu()
...
def shoot_magazine(): # GOOD ENDING
    ...
    stop_timer()
    text_id = display("You shot his right knee delaying his chase. You took this opportunity to escape. With one final burst of energy, you leap towards the exit and find yourself back in the safety of daylight. The creature's wail fade into silence. YOU ESCAPED!")
    canvas.itemconfig(text_id, font=('Times New Roman', 18, 'bold'), fill='gold')
    end_menu()            
...
def riddle_2(): # NOT TIMED
    ...
    button_A = buttons(window, text='Read', command=riddle_2_choices, x=450, y=350)

def right_path(): #NOT TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image25.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    stop_timer()
    text_id = display("You walked into a wide-open chamber. There are two exits.")
    canvas.itemconfig(text_id, fill='black')
    canvas.coords(text_id, 170, 100)
    button_A = buttons(window, text='LEFT', command=riddle_2, x=250, y=400)
    button_B = buttons(window, text='RIGHT', command=silence, x=600, y=400)    

def search(): # NOT TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image7.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    stop_timer()
    display("You saw a loaded magazine lying on the ground.")
    button_A = buttons(window, text='Pick it up', command=pick_magazine, x=250, y=400)
    button_B = buttons(window, text='Leave it', command=leave_magazine, x=600, y=400)  

def walk_deeper(): # NOT TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image6.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    stop_timer()
    display("You stumble upon a gun lying on the ground. You pick it up feeling its weight.")
    button_A = buttons(window, text='LEFT', command=left_path2, x=250, y=400)
    button_B = buttons(window, text='RIGHT', command=search, x=600, y=400)
...
def check_riddle(): # NOT TIMED
    ...
    if user_answer == "CHASE":
        ...
        proceed = Button(window, text='I will escape', command=walk_deeper, font=('Times New Roman', 20, 'bold'), fg='gold', bg='black', activeforeground='white', activebackground='gray', relief=RAISED, borderwidth=5, padx=5, pady=5)
...
def riddle(): # NOT TIMED
    ...
    clear_dynamic_widgets()  
    canvas.delete('all') 
    photo = PhotoImage(file='Image8.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')
    ...
    riddle = Label(window, text='"I am the echo of your footsteps, the whisper of your fear. I am always one step behind, and I will never let you escape. I am a five-letter word, what am I?"',
                          font=('Brush Script MT', 40), fg='gold', bg='black', wraplength=850, relief=RAISED, borderwidth=5, justify='center', padx=10, pady=5)
    ...
    submit_button = Button(window, text='Submit', font=('Cambria', 15,), fg='white', bg='gray', command=check_riddle)
...
def turnback(): # NOT TIMED
    ...
    button_A = buttons(window, text='Alright', command=riddle, x=400, y=350)
    button_B = buttons(window, text='I change my mind.', command=walk_deeper, x=320, y=450)

def left_path(): # NOT TIMED
    global photo
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image15.png')
    canvas.create_image(0, 0, image=photo,anchor='nw')

    display("You enter a narrowly dimly lit corridor. The walls are damp and cold.")
    button_A = buttons(window, text='Walk in Deeper', command=walk_deeper, x=150, y=400)
    button_B = buttons(window, text='Turnback', command=turnback, x=600, y=400)
...
def maze_start(): # NOT TIMED
    ...
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image14.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    display("You find yourself at the entrance of a dark labyrinth. Which path will you choose?")
    button_A = buttons(window, text='LEFT', command=left_path, x=250, y=400)
    button_B = buttons(window, text='RIGHT', command=right_path, x=600, y=400)
...
def scrolling_text():
    ...
    text_id = canvas.create_text(500, 700, text=credits_text, font=('Cambria',20), fill='white', justify='center')

    def scroll_animation():
        nonlocal text_id
        canvas.move(text_id, 0, -2)
        current_position = canvas.coords(text_id)[1]
        if current_position < -300:
            canvas.coords(text_id, 500, 700)
...
def check_sample_riddle():
    ...
    if answer_sample == "shadow":
        ...
        start_button = Button(window, text='Start', command=maze_start,
...
def next():
    ...
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='Image9.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')

    text_id = display("You might also stumble upon riddles which requires you to answer it like this:")
    canvas.itemconfig(text_id, font=('Times New Roman', 18, 'bold'))
    canvas.coords(text_id, 110, 60)
    ...
    riddle_sample = Label(window, text='"I follow you closely, yet I have no form. In light, I grow longer, in darkness, I swarm. I twist through the pathways, both silent and near. What am I that brings both dread and fear?"',
                          font=('Lucida Calligraphy', 20), fg='black', bg='#7e8260', wraplength=850, relief=RAISED, borderwidth=5, justify='center', padx=10, pady=5)
    ...
    submit_button = Button(window, text='Submit', font=('Cambria', 15,), fg='white', bg='gray', command=check_sample_riddle)
...
def timer_tutorial():
    ...
    text_id = display("Every choice must be swift. Hesitate too long, and the shadows may catch up.")
    canvas.itemconfig(text_id, font=('Times New Roman', 30, 'bold'))
    canvas.coords(text_id, 80, 90)
    ...
    button_A = buttons(window, text='Back', command=how_to_play, x=250, y=500)
    button_B = buttons(window, text='Next', command=next, x=600, y=500)
...
def how_to_play():
    ...
    text_id = display("A scenario will be given and you have to choose one of the choices.")
    canvas.coords(text_id, 140, 150)
...
def title_screen():
    ...
    button1 = Button(window, text='New path', command=maze_start, font=('Wide Latin', 18), bg='gray', fg='black', relief=RAISED)
    button1.bind("<Enter>", on_enter)
    button1.bind("<Leave>", on_leave)
    button1_window = canvas.create_window(500, 295, window=button1)  

    button2 = Button(window, text='How to Play', command=how_to_play, font=('Wide Latin', 18), bg='gray', fg='black', relief=RAISED)
    button2.bind("<Enter>", on_enter)
    button2.bind("<Leave>", on_leave)
    button2_window = canvas.create_window(500, 360, window=button2)

    button3 = Button(window, text='Credits', command=scrolling_text, font=('Wide Latin', 18), bg='gray', fg='black', relief=RAISED)
    button3.bind("<Enter>", on_enter)
    button3.bind("<Leave>", on_leave)
    button3_window = canvas.create_window(500, 425, window=button3)

    button4 = Button(window, text='Quit', command=window.destroy, font=('Wide Latin', 18), bg='gray', fg='black', relief=RAISED)
