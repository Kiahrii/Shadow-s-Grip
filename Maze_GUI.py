...
main_background_music = pygame.mixer.Sound('MusicBox.mp3')
button_click = pygame.mixer.Sound('Button Click.mp3')
timer_sound = pygame.mixer.Sound('Ticktock.mp3')
timer_music = pygame.mixer.Sound('Dun.mp3')
...
def tick_loop():
    global is_clock_ticking
    if not is_clock_ticking:
        timer_sound.play(loops=-1)
...
def stop_timer():
    global my_timer, timer_id, is_clock_ticking
    if timer_id is not None:
        window.after_cancel(timer_id)
        print("Timer stopped")
        timer_sound.stop()
...
def left_path(): # NOT TIMED
    ...
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='woods3.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')
...
def maze_start(): # NOT TIMED
    ...
    clear_dynamic_widgets()
    canvas.delete('all')
    photo = PhotoImage(file='woods8.png')
    canvas.create_image(0, 0, image=photo, anchor='nw')
